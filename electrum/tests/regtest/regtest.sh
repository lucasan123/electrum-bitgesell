...
bitgesell_cli="bitgesell-cli -rpcuser=doggman -rpcpassword=donkey -rpcport=18554 -regtest"

function new_blocks()
{
    $bitgesell_cli generatetoaddress $1 $($bitgesell_cli getnewaddress) > /dev/null
}
...
function wait_until_spent()
{
    ...
    while [[ $($bitgesell_cli gettxout $1 $2) ]]; do
        sleep 1
	msg="$msg."
	printf "$msg\r"
    done
...
if [[ $1 == "init" ]]; then
	...
	$bob setconfig --offline lightning_listen localhost:9735
        ...
        echo "funding $2"
        $bitgesell_cli sendtoaddress $($agent getunusedaddress -o) 1
    fi
fi
...
if [[ $1 == "breach" ]]; then
    ...
    $bitgesell_cli sendrawtransaction $ctx
    new_blocks 1
...
if [[ $1 == "breach_with_spent_htlc" ]]; then
    ...
    $bob stop
    ctx_id=$($bitgesell_cli sendrawtransaction $ctx)
    echo "alice breaches with old ctx:" $ctx_id
    new_blocks 1
    if [[ $($bitgesell_cli gettxout $ctx_id 0 | jq '.confirmations') != "1" ]]; then
