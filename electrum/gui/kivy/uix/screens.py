...
from electrum import bitgesell, constants
from electrum.transaction import Transaction, tx_from_any, PartialTransaction, PartialTxOutput
from electrum.util import (parse_URI, InvalidBitgesellURI, PR_PAID, PR_UNKNOWN, PR_EXPIRED,
...
class SendScreen(CScreen):
    ...
    def set_ln_invoice(self, invoice):
        ...
        self.amount = self.app.format_amount_and_units(lnaddr.amount * bitgesell.COIN) if lnaddr.amount else ''
        self.payment_request = None
    ...
    def read_invoice(self):
        ...
        if not address:
            self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Bitgesell address or a payment request'))
            return
        if not self.amount:
        ...
        if self.is_lightning:
            ...
            if self.payment_request:
                ...
                if not bitgesell.is_address(address):
                    self.app.show_error(_('Invalid Bitgesell Address') + ':\n' + address)
                    return
