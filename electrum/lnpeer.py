...
class Peer(Logger):
    ...
    def on_announcement_signatures(self, chan: Channel, payload):
        if chan.config[LOCAL].was_announced:
            h, local_node_sig, local_bitgesell_sig = self.send_announcement_signatures(chan)
        else:
    ...
    def verify_channel_announcements(self, chan_anns):
        for payload in chan_anns:
            ...
            pubkeys = [payload['node_id_1'], payload['node_id_2'], payload['bitgesell_key_1'], payload['bitgesell_key_2']]
            sigs = [payload['node_signature_1'], payload['node_signature_2'], payload['bitgesell_signature_1'], payload['bitgesell_signature_2']]
    ...
    async def channel_establishment_flow(
        ...
        funding_address = bitgesell.redeem_script_to_address('p2wsh', redeem_script)
    ...
    async def handle_announcements(self, chan: Channel):
        ...
        remote_bitgesell_sig = announcement_signatures_msg["bitgesell_signature"]
        if not ecc.verify_signature(chan.config[REMOTE].multisig_key.pubkey, remote_bitgesell_sig, h):
            raise Exception("bitgesell_sig invalid in announcement_signatures")
        if not ecc.verify_signature(self.pubkey, remote_node_sig, h):
            raise Exception("node_sig invalid in announcement_signatures")

        node_sigs = [remote_node_sig, local_node_sig]
        bitgesell_sigs = [remote_bitgesell_sig, local_bitgesell_sig]
        bitgesell_keys = [chan.config[REMOTE].multisig_key.pubkey, chan.config[LOCAL].multisig_key.pubkey]

        if self.node_ids[0] > self.node_ids[1]:
            node_sigs.reverse()
            bitgesell_sigs.reverse()
            node_ids = list(reversed(self.node_ids))
            bitgesell_keys.reverse()
        else:
        ...
        self.send_message("channel_announcement",
            ...
            bitgesell_signature_1=bitgesell_sigs[0],
            bitgesell_signature_2=bitgesell_sigs[1],
            len=0,
            ...
            bitgesell_key_1=bitgesell_keys[0],
            bitgesell_key_2=bitgesell_keys[1]
        )
    ...
    def send_announcement_signatures(self, chan: Channel):
        ...
        bitgesell_signature = ecc.ECPrivkey(chan.config[LOCAL].multisig_key.privkey).sign(msg_hash, sig_string_from_r_and_s)
        ...
        self.send_message("announcement_signatures",
            ...
            bitgesell_signature=bitgesell_signature
        )
        return msg_hash, node_signature, bitgesell_signature
    ...
    def on_update_add_htlc(self, chan: Channel, payload):
        ...
        if cltv_expiry > bitgesell.NLOCKTIME_BLOCKHEIGHT_MAX:
            asyncio.ensure_future(self.lnworker.try_force_closing(chan.channel_id))
    ...
    async def maybe_update_fee(self, chan: Channel):
        ...
        if not chan.constraints.is_initiator:
            if constants.net is not constants.BitgesellRegtest:
    ...
    async def send_shutdown(self, chan: Channel):
        ...
        scriptpubkey = bfh(bitgesell.address_to_script(chan.sweep_address))
    ...
    async def _shutdown(self, chan: Channel, payload, is_local):
        ...
        chan.set_can_send_ctx_updates(False)
        their_scriptpubkey = payload['scriptpubkey']
        our_scriptpubkey = bfh(bitgesell.address_to_script(chan.sweep_address))
