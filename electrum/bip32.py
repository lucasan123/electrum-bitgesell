...
from .util import bfh, bh2u, BitgesellException
from . import constants
from . import ecc
from .crypto import hash_160, hmac_oneshot
from .bitgesell import rev_hex, int_to_hex, EncodeBase58Check, DecodeBase58Check
...
def _CKD_priv(parent_privkey: bytes, parent_chaincode: bytes,
    ...
    try:
        ...
        raise BitgesellException('Impossible xprv (not within curve order)') from e
...
class InvalidMasterKeyVersionBytes(BitgesellException): pass
...
class BIP32Node(NamedTuple):
    ...
    def from_xkey(cls, xkey: str, *, net=None) -> 'BIP32Node':
        ...
        if len(xkey) != 78:
            raise BitgesellException('Invalid length for extended key: {}'
                                   .format(len(xkey)))
        depth = xkey[4]
    ...
    def from_rootseed(cls, seed: bytes, *, xtype: str) -> 'BIP32Node':
        I = hmac_oneshot(b"Bitgesell seed", seed, hashlib.sha512)
        master_k = I[0:32]
        master_c = I[32:]
        return BIP32Node(xtype=xtype,
