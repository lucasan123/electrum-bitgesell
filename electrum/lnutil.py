...
from . import ecc, bitgesell, crypto, transaction
from .bitgesell import push_script, redeem_script_to_address, address_to_script
...
def make_htlc_tx_output(amount_msat, local_feerate, revocationpubkey, local_delayedpubkey, success, to_self_delay):
    ...
    script = bytes([opcodes.OP_IF]) \
        ...
        + bitgesell.add_number_to_script(to_self_delay) \
    ...
    p2wsh = bitgesell.redeem_script_to_address('p2wsh', bh2u(script))
...
def make_offered_htlc(revocation_pubkey: bytes, remote_htlcpubkey: bytes,
    ...
    return bytes([opcodes.OP_DUP, opcodes.OP_HASH160]) + bfh(push_script(bh2u(bitgesell.hash_160(revocation_pubkey))))\
        + bytes([opcodes.OP_EQUAL, opcodes.OP_IF, opcodes.OP_CHECKSIG, opcodes.OP_ELSE]) \
        + bfh(push_script(bh2u(remote_htlcpubkey)))\
        + bytes([opcodes.OP_SWAP, opcodes.OP_SIZE]) + bitgesell.add_number_to_script(32) + bytes([opcodes.OP_EQUAL, opcodes.OP_NOTIF, opcodes.OP_DROP])\
        + bitgesell.add_number_to_script(2) + bytes([opcodes.OP_SWAP]) + bfh(push_script(bh2u(local_htlcpubkey))) + bitgesell.add_number_to_script(2)\
...
def make_received_htlc(revocation_pubkey: bytes, remote_htlcpubkey: bytes,
    ...
    return bytes([opcodes.OP_DUP, opcodes.OP_HASH160]) \
        + bfh(push_script(bh2u(bitgesell.hash_160(revocation_pubkey)))) \
        + bytes([opcodes.OP_EQUAL, opcodes.OP_IF, opcodes.OP_CHECKSIG, opcodes.OP_ELSE]) \
        + bfh(push_script(bh2u(remote_htlcpubkey))) \
        + bytes([opcodes.OP_SWAP, opcodes.OP_SIZE]) \
        + bitgesell.add_number_to_script(32) \
...
def make_commitment_outputs(*, fees_per_participant: Mapping[HTLCOwner, int], local_amount_msat: int, remote_amount_msat: int,
    ...
    for script, htlc in htlcs:
        addr = bitgesell.redeem_script_to_address('p2wsh', bh2u(script))
...
def make_commitment_output_to_local_witness_script(
    ...
    local_script = bytes([opcodes.OP_IF]) + bfh(push_script(bh2u(revocation_pubkey))) + bytes([opcodes.OP_ELSE]) + bitgesell.add_number_to_script(to_self_delay) \
...
def make_commitment_output_to_local_address(
    ...
    return bitgesell.redeem_script_to_address('p2wsh', bh2u(local_script))

def make_commitment_output_to_remote_address(remote_payment_pubkey: bytes) -> str:
    return bitgesell.pubkey_to_address('p2wpkh', bh2u(remote_payment_pubkey))
