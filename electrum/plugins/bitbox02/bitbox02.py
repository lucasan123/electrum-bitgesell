...
from electrum.bitgesell import OnchainOutputType

import electrum.bitgesell as bitgesell
...
class BitBox02Client(HardwareClientBase):
    ...
    def coin_network_from_electrum_network(self) -> int:
        if constants.net.TESTNET:
            return bitbox02.btc.TBGL
        return bitbox02.btc.BGL
    ...
    def get_password_for_storage_encryption(self) -> str:
        ...
        node = bip32.BIP32Node.from_xkey(xpub, net = constants.BitgesellMainnet()).subkey_at_public_derivation(())
    ...
    def get_xpub(self, bip32_path: str, xtype: str, *, display: bool = False) -> str:
        ...
        if xtype == "p2wpkh":
            if coin_network == bitbox02.btc.BGL:
                out_type = bitbox02.btc.BGLPubRequest.ZPUB
            else:
                out_type = bitbox02.btc.BGLPubRequest.VPUB
        elif xtype == "p2wpkh-p2sh":
            if coin_network == bitbox02.btc.BGL:
                out_type = bitbox02.btc.BGLPubRequest.YPUB
            else:
                out_type = bitbox02.btc.BGLPubRequest.UPUB
        elif xtype == "p2wsh":
            if coin_network == bitbox02.btc.BGL:
                out_type = bitbox02.btc.BGLPubRequest.CAPITAL_ZPUB
            else:
                out_type = bitbox02.btc.BGLPubRequest.CAPITAL_VPUB
    ...
    def btc_multisig_config(
        ...
        multisig_config = bitbox02.btc.BGLScriptConfig(
            multisig=bitbox02.btc.BGLScriptConfig.Multisig(
                threshold=wallet.m,
    ...
    def show_address(
        ...
        if address_type == "p2wpkh":
            script_config = bitbox02.btc.BGLScriptConfig(
                simple_type=bitbox02.btc.BGLScriptConfig.P2WPKH
            )
        elif address_type == "p2wpkh-p2sh":
            script_config = bitbox02.btc.BGLScriptConfig(
                simple_type=bitbox02.btc.BGLScriptConfig.P2WPKH_P2SH
            )
    ...
    def sign_transaction(
        ...
        coin = bitbox02.btc.BGL
        if constants.net.TESTNET:
            coin = bitbox02.btc.TBGL

        tx_script_type = None

        # Build BGLInputType list
        inputs = []
        ...
        if tx_script_type == "p2wpkh":
            tx_script_type = bitbox02.btc.BGLScriptConfig(
                simple_type=bitbox02.btc.BGLScriptConfig.P2WPKH
            )
        ...
        for txout in tx.outputs():
            ...
            if txout.is_change:
                ...
                outputs.append(
                    bitbox02.BGLOutputInternal(
                        keypath=change_pubkey_path, value=txout.value,
                    )
                )
            else:
                addrtype, pubkey_hash = bitgesell.address_to_hash(txout.address)
