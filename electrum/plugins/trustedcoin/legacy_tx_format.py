...
from electrum.electrum import push_script, int_to_hex, var_int
...
def get_xpubkey(keystore: BIP32_KeyStore, c, i) -> str:
    def encode_path_int(path_int) -> str:
        if path_int < 0xffff:
            hex = electrum.int_to_hex(path_int, 2)
        else:
            hex = 'ffff' + electrum.int_to_hex(path_int, 4)
        return hex

    s = ''.join(map(encode_path_int, (c, i)))
    return 'ff' + electrum.DecodeBase58Check(keystore.xpub).hex() + s
