...
class CoinGecko(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coingecko.com', '/api/v3/simple/price?ids=bitgesell&vs_currencies=%s' % ccy)
        return {ccy.upper(): Decimal(json['bitgesell'][ccy.lower()])}
    ...
    async def request_history(self, ccy):
        history = await self.get_json('api.coingecko.com',
                                      '/api/v3/coins/bitgesell/market_chart?vs_currency=%s&days=max' % ccy)
